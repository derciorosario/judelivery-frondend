...
const MotoristaHistory = ({ refreshKey }) => {
  ...
  useEffect(() => {
     const fetchOrders = async () => {
       setLoading(true);
       try {
         const response = await getDriverOrders({ status: "completed", limit: 50 });
  ...
  const averageRating = orders.length > 0 
    ? (orders.reduce((sum, o) => sum + (o.feedbacks?.[0]?.rating || 0), 0) / orders.length).toFixed(1)
    : 0;
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs text-slate-500">{totalOrders} entregas</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        ...
        <div className="bg-white rounded-xl p-3 text-center border border-slate-100 shadow-sm">
          <p className="text-lg font-bold text-slate-800">{totalEarnings.toFixed(0)} MZN</p>
          <p className="text-[10px] text-slate-400">Ganhos Totais</p>
        </div>
        <div className="bg-white rounded-xl p-3 text-center border border-slate-100 shadow-sm">
          <p className="text-lg font-bold text-slate-800 flex items-center justify-center gap-0.5">
            {averageRating || "0"} <Icon name="star" size={14} className="text-amber-400" />
          </p>
      ...
      {loading ? (
        ...
        <div className="text-center py-10">
          ...
          <p className="text-sm text-slate-500">Nenhuma entrega concluída encontrada</p>
          <p className="text-xs text-slate-400 mt-1">Suas entregas concluídas aparecerão aqui</p>
        </div>
      )}
